...
def post_comment(request, post_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # 检查到数据是合法的，调用表单的save方法保存数据导数据库
            # commit=False 的作用是仅仅利用表单的数据生成Comment模型类的实例，还不保存评论数据到数据库
            ...
            # 重定向到post的详情页，实际上当redirect函数接受一个模型实例时，它会调用这个模型实例的 get_absolute_url 方法
            ...
            # 注意到这里我们用到了post.comment_set.all()方法
            # 其作用是获取这篇post下的全部评论
    ...
    # 不是post请求，说明用户没有提交数据，重新定向到文章详情页
    return redirect(post)
